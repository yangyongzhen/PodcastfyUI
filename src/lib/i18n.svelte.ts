...
const EN: Record<string, string> = {
  ...
  "主题搜索": "Topic search",
  "搜索后端": "Search backend",
  "auto（按可用性自动选择）": "auto (first backend that answers)",
  "Exa（免密钥可用）": "Exa (works without a key)",
  "博查（国内）": "BoCha (China)",
  "智谱（国内）": "Zhipu (China)",
  "千帆 / 百度（国内）": "Qianfan / Baidu (China)",
  "结果条数": "Results per search",
  "搜索全部失败时，用模型自带知识继续生成（转录稿会标注未经联网检索）":
    "If every search backend fails, keep generating from the model's own knowledge (the transcript is marked as not web-checked)",
  "Exa 密钥（可空）": "Exa API key (optional)",
  "不填也能搜，填了提高配额": "Leave blank to search anyway; a key raises the quota",
  "Serper 密钥（可空）": "Serper API key (optional)",
  "博查密钥（可空）": "BoCha API key (optional)",
